...
-- Get count of horses by running style (for statistics/dashboard)
SELECT style, COUNT(*) AS horse_count
FROM Horses
GROUP BY style
ORDER BY horse_count DESC;

-- Get count of horses by preferred surface
SELECT preferred_race_surface, COUNT(*) AS horse_count
FROM Horses
GROUP BY preferred_race_surface;

-- Get count of horses by preferred distance
SELECT preferred_race_distance, COUNT(*) AS horse_count
FROM Horses
GROUP BY preferred_race_distance
ORDER BY FIELD(preferred_race_distance, 'Short', 'Mile', 'Medium', 'Long');

-- Get races with number of entries
SELECT r.race_id, r.name, r.surface_type, r.distance, COUNT(rh.horse_id) AS entry_count
FROM Races r
LEFT JOIN RacesHorses rh ON r.race_id = rh.race_id
GROUP BY r.race_id, r.name, r.surface_type, r.distance
ORDER BY entry_count DESC;

-- Get horses with number of sparks assigned
SELECT h.horse_id, h.name, COUNT(hs.spark_id) AS spark_count
FROM Horses h
LEFT JOIN HorsesSparks hs ON h.horse_id = hs.horse_id
GROUP BY h.horse_id, h.name
ORDER BY spark_count DESC;

-- Find horses with specific stat preferences matching a race
SELECT h.horse_id, h.name, h.style, h.base_speed, h.base_stamina
FROM Horses h
WHERE h.preferred_race_distance = :race_distance 
  AND h.preferred_race_surface = :race_surface
ORDER BY h.name;