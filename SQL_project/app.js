...
app.get('/races-horses/edit/:id', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM RacesHorses WHERE race_horse_id = ?', [req.params.id]);
...
app.get('/horses-sparks/edit/:id', async (req, res) => {
    try {
        ...
        res.render('horses-sparks_edit', { title: 'Edit Horses to Sparks', assignment, horses, sparks });
    } catch (err) {
        console.error(err);
